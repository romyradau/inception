...
#ich bin mir bewusst dass ich root bin!
...
wp core config --path=/var/www/wordpress --allow-root --dbname=${DB_NAME} --dbuser=${DB_USER} --dbpass=${DB_USER_PASSWORD} --dbhost=${HOST_NAME} \
# wp config create --path=/var/www/wordpress --allow-root --dbname=${DB_NAME} --dbuser=${DB_USER} --dbpass=${DB_USER_PASSWORD} --dbhost=${HOST_NAME}
...
wp core install --path=/var/www/wordpress --allow-root --url=${WP_ADMIN_URL} --title=${WP_TITLE} --admin_user=${DB_USER} --admin_password=${DB_USER_PASSWORD} --admin_email=rschleic@student.42heilbronn.de
...
wp user create --path=/var/www/wordpress --allow-root ${WP_USER} ${WP_USER_MAIL} --role=editor --first_name=roxx --last_name=slr --user_pass=${WP_USER_PASSWORRD}
